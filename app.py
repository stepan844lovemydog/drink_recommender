...
<style>
    ...
    /* Стиль для сайдбара (прогресс) */
    .progress-text {
        font-size: 0.9rem;
        color: #666;
        text-align: center;
    }
    /* Эффект свечения для результата */
    .glow {
...
with st.sidebar:
    st.image("https://emojigraph.org/media/apple/cocktail-glass_1f379.png", width=80)
    st.markdown("### 🧠 Как это работает")
    st.markdown("""
    1. Ответь на 11 вопросов  
    2. Нажми кнопку  
    3. Нейросеть подберёт идеальный напиток  
    """)
    st.progress(0.0)  # будет обновляться позже
...
variants = {
    'Что бы вы хотелимо сейчас?': ['Освежающее', 'Расслабляющее', 'Бодрящее'],
...
# ---- ВЫВОД ВОПРОСОВ (в стильных карточках) ----
answers = {}
...
if st.button("🍹 ПОДОБРАТЬ НАПИТОК"):
    ...
    # ========== ОСНОВНАЯ ЛОГИКА (как в прошлом коде) ==========
